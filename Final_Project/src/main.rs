...
fn main() {
    ...
    match subcommand.as_str() {
        ...
        "blur" => {
            ...
            let amount: f32 = amount.parse().expect("Failed to parse a number");
            // **OPTION**
        ...
        "brighten" => {
            ...
            let outfile = args.remove(0);
            let amount = args.remove(0);
            let amount: i32 = amount.parse().expect("Failed to parse a number");
        ...
        "generate" => {
            ...
            let outfile = args.remove(0);
            let red = args.remove(0).parse().expect("Failed to parse a number");
            let green = args.remove(0).parse().expect("Failed to parse a number");
            let blue = args.remove(0).parse().expect("Failed to parse a number");
...
fn generate(outfile: String, red: u8, green: u8, blue: u8) {
    ...
    let graph_r = 255 - red;
    let graph_g = 255 - green;
    let graph_b = 255 - blue;
    ...
    let x = (x as f32) * scale_y - 1.5;
    let y = (y as f32) * scale_x - 1.5;
    //println!("({}, {})", x,y);
    let c = num_complex::Complex::new(x, y);
    //println!("z norm: {}", z.norm());
    let mut z = num_complex::Complex::new(0 as f32, 0 as f32);
     let mut i = 0;
     while i<100 && z.norm() <= 2.0{
         z = z*z+c;
         i += 1;
     }
     //println!("({},{}) norm: {}",x,y, z.norm());

    if z.norm() <= 2.0{
        *pixel = image::Rgb([graph_r, graph_g, graph_b]);    
    }else{
